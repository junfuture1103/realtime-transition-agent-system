...
- 배포 환경의 `world_bot`은 서버 시작 시 자동 실행되고, 3,000건 공개용 dataset을 초당 1건씩 순환 replay한다.
- 상단 지표는 거래 발생 수, 이상거래 탐지율, 정답 공개 수, 모델 일치율, 공격 성공률을 보여준다.
- 거래 탭에는 초 단위 stream chart와 관리자 공개 라벨 기반 comparison chart가 있다.
- 정답 라벨은 `dataset_truth_labels`에 숨겨 저장되고, `POST /api/admin/labels/reveal` 또는 관리자 UI 버튼을 누른 row만 `transactions.label`에 공개된다.
...
- directory: Docker volume path `/app/data/generated/realtime_financial_transactions_3000/`
- manifest: `/app/data/generated/realtime_financial_transactions_3000/manifest.json`
- complete shards: `1`
- rows per shard: `3,000`
- complete rows: `3,000`
- profile count: `3,000`
- timeline: `2025-01-01T00:00:00` across about `7` days
- class balance: normal `2,946`, fraud `54`
- compressed size: about `242KB`
- replay policy: `FRAUD_LAB_BOT_AUTO_START=true`, `FRAUD_LAB_BOT_INTERVAL_SECONDS=1`, `FRAUD_LAB_BOT_BATCH_SIZE=1`, `FRAUD_LAB_BOT_LOOP_DATASET=true`
- label policy: truth labels are hidden until explicit admin reveal; public CSV only shows `revealed_label` after reveal.

Host-local full dataset, not exposed by the web app:

- directory: `/home/cau/realtime-transition-agent-system/data/generated/realtime_financial_transactions_10000000/`
- manifest: `/home/cau/realtime-transition-agent-system/data/generated/realtime_financial_transactions_10000000/manifest.json`
- complete shards: `10`
...
- this path is ignored by git and is not mounted into the running web app container.
